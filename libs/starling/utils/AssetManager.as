package starling.utils
{
    ...
    public class AssetManager
    {
        private const SUPPORTED_EXTENSIONS:Vector.<String> = 
            new <String>["png", "jpg", "jpeg", "atf", "mp3", "xml", "fnt", "json"]; 
		...
		private var mBitmapData:Dictionary;
		private var mAtlasesXMLs:Dictionary;
        private var mTextures:Dictionary;
        private var mAtlases:Dictionary;
        private var mSounds:Dictionary;
        private var mJSONData:Dictionary;
        ...
        public function AssetManager(scaleFactor:Number=-1, useMipmaps:Boolean=false)
        {
            ...
            mRawAssets = [];
			mJSONData = new Dictionary();
			mBitmapData = new Dictionary();
            mTextures = new Dictionary();
            mAtlases = new Dictionary();
            mSounds = new Dictionary();
			mAtlasesXMLs = new Dictionary();
        }
		...
		/** Returns a bitmapdata with a certain name, or null if it's not found. */
		public function getBitmapData(name:String):BitmapData {
			return mBitmapData[name] as BitmapData;
		}
		
		public function getAtlasXml(name:String):XML
		{
			return mAtlasesXMLs[name] as XML;
		}
		...
		/** Register a bitmapData under a certain name. It will be availble right away. */
		public function addBitmapData(name:String, bitmapData:BitmapData):void
		{
			log("Adding bitmapData'" + name + "'");
			
			if (name in mBitmapData)
				throw new Error("Duplicate bitmapData name: " + name);
			else
				mBitmapData[name] = bitmapData;
			
		}
		
		/** Register an XML under a certain name. It will be available right away. */		
		public function addAtlasXml(name:String, xml:XML):void
		{
			log("Adding atlas xml'" + name + "'");
			
			if (name in mAtlasesXMLs) 
				throw new Error("Duplicate xml found name: " + name);
			else
				mAtlasesXMLs[name] = xml;
		}
		...
		public function addJSON(name:String, json:String):void {
			log("Adding JSON '" + name + "'");
			
			if (name in mJSONData) 
				throw new Error("Duplicate JSON name: " + name);
			else
				mJSONData[name] = json;
		...
		/** Removes a certain BitmapData. */
		public function removeBitmapData(name:String, dispose:Boolean=true):void {
			if (dispose && name in mBitmapData)
				mBitmapData[name].dispose();
			
			delete mBitmapData[name];
			
		}
		
		/** Removes a certain XML. */
		public function removeXML(name:String):void {
			delete mAtlasesXMLs[name];
		}
		
		/** Removes a certain JSON. */
		public function removeJSON(name:String):void {
			delete mJSONData[name];
		}
        ...
        public function purge():void
        {
			...
			for each (var bitmapData:BitmapData in mBitmapData)
				bitmapData.dispose();
                                      ...
                                      onProgress:Function, onComplete:Function):void
        {
            ...
            if (rawAsset is Class)
            {
                ...
                {
					addBitmapData(name, Bitmap(asset).bitmapData.clone());
            ...
            function onLoaderComplete(event:Event):void
            {
                ...
                if (content is Bitmap) {
					addBitmapData(name, Bitmap(content).bitmapData.clone());
                    addTexture(name,
